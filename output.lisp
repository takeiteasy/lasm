...
(defun assembly-bytes (assembly &key machine memory endian)
...
not a multiple of 8."
  (let* ((n (%output-cell-bytes (assembly-cell-width assembly)))
         ...
         (cells (or (assembly-cells assembly) #()))
...
(defun write-binary (assembly path &key machine memory endian)
...
(defun hex-text (assembly &key stream machine memory endian)
...
labels. Keys are ASSEMBLY-BYTES'. Returns the text as a string when STREAM is
NIL (default); otherwise writes to STREAM and returns NIL."
  (let* ((n (%output-cell-bytes (assembly-cell-width assembly)))
         ...
         (start (* n (assembly-origin assembly)))
...
(defun write-intel-hex (assembly path &key machine memory endian)
