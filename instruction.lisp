...
(defun %literal-truth (form)
  ...
  (cond ((null form) :false)
        ((or (eq form t) (keywordp form) (numberp form) (stringp form)) :true)
...
IF, or of a WHEN, UNLESS, AND or COND, testing it, or its NOT or NULL."
  (let* ((descriptor (find-machine-descriptor machine))
    ...
    (labels ((note (symbol conditions)
             ...
             (clause-conditions (spec clauses)
               (let ((name (if (consp spec) (first spec) spec))
                     (steps (and (consp spec) (rest spec) (ignore-errors (%prefix-steps (rest spec)))))
                     (seen '())
                     (result '()))
                 (dolist (clause clauses (nreverse result))
                   (when (consp clause)
                     (let ((head (first clause)))
                       (if (member head '(otherwise t))
                           (cl:push (cons clause (list name steps :not-in seen)) result)
                           (let ((keys (if (listp head) head (list head))))
                             (cl:push (cons clause (list name steps :in (remove-if (lambda (key) (member key seen)) keys)))
                                      result)
             ...
             (walk-list (forms conditions vars)
               (loop for tail = forms then (cdr tail)
                     while (consp tail)
                     do (walk (car tail) conditions vars))))
      (walk-list forms '() '()))
    (nreverse writes)))
