...
(fiveam:test defbackend-signals-a-typed-error-naming-the-definition
  (dolist (form '((defbackend bk-bad-1 (:machine no-such-machine))
                  ...
                  (defbackend bk-bad-24 (:machine callfoo) (ops (:exchange (a) (ret))))))
...
(defmachine bk-two-cells-machine
  (register pc :width 16)
  (memory narrow :width 8 :addr-width 16)
  (memory wide :width 16 :addr-width 16))

(definstruction bk-two-cells-machine lda
  (modes
    (zero-page (opcode #x11) (operand v :width 1) (semantics nil))
    (absolute (opcode #x12) (operand v :width 2) (semantics nil))))
...
(fiveam:test an-extending-backend-merges-the-parent-clauses-by-key
  (let ((child (find-backend 'callfoo-fp-abi)))
    ...
    (fiveam:is (equal '(:grows :down :alignment 1 :slot "FP-IDX" :stack-slot "SP-IDX" :pointer "FP") (backend-descriptor-frame child)))
...
(fiveam:test a-child-backend-follows-later-changes-to-its-parent
  (eval '(defbackend bk-snap-parent (:machine callfoo) (registers :return (a))))
  (eval '(defbackend bk-snap-child (:extends bk-snap-parent) (registers :scratch (b))))
  (eval '(defbackend bk-snap-grandchild (:extends bk-snap-child)))
  (eval '(defbackend bk-snap-parent (:machine callfoo) (registers :return (c))))
  (fiveam:is (equal '("C") (backend-register 'bk-snap-child :return)))
  (fiveam:is (equal '("B") (backend-register 'bk-snap-child :scratch)))
  (fiveam:is (equal '("C") (backend-register 'bk-snap-grandchild :return)))
  (fiveam:is (eq 'bk-snap-child (backend-descriptor-parent (find-backend 'bk-snap-grandchild)))))

(fiveam:test a-child-that-no-longer-builds-leaves-every-backend-as-it-was
  (eval '(defbackend bk-keep-parent (:machine callfoo)
          (registers :return (a))
          (operands (reg call-reg) (imm call-imm))
          (ops (:push (x) (pushv x)))))
  (eval '(defbackend bk-keep-child (:extends bk-keep-parent) (ops (:pop (x) (popr (imm x))))))
  (let ((detail (%backend-error-of '(defbackend bk-keep-parent (:machine callfoo)
                                     (registers :return (b))
                                     (ops (:push (x) (pushv x)))))))
    (fiveam:is (typep detail 'backend-definition-error))
    (fiveam:is (search "bk-keep-child" (string-downcase (princ-to-string detail)))))
  (fiveam:is (equal '("A") (backend-register 'bk-keep-parent :return)))
  (fiveam:is (equal '("A") (backend-register 'bk-keep-child :return))))

(fiveam:test a-backend-cannot-extend-one-that-extends-it
  (eval '(defbackend bk-cycle-a (:machine callfoo)))
  (eval '(defbackend bk-cycle-b (:extends bk-cycle-a)))
...
(fiveam:test branches-and-stack-effects-are-checked
  (dolist (form '((defbackend bk-br-e1 (:machine callfoo) (branches nope))
                  (defbackend bk-br-e2 (:machine callfoo) (branches call call))
                  (defbackend bk-br-e3 (:machine callfoo) (branches 3))
                  (defbackend bk-br-e4 (:machine callfoo) (registers :operand reg) (operands (reg call-reg))
                   (ops (:grab (n) :pops m (hlt))))
                  (defbackend bk-br-e5 (:machine callfoo) (operands (reg call-reg))
                   (ops (:grab (n) :sideways 1 (hlt))))
                  (defbackend bk-br-e6 (:machine callfoo) (operands (reg call-reg))
                   (ops (:grab (n) :pops 1 :pops 2 (hlt))))
                  (defbackend bk-br-e7 (:machine callfoo) (operands (reg call-reg))
                   (ops (:push (x) :pushes 1 (hlt))))))
    (fiveam:is (typep (%backend-error-of form) 'backend-definition-error) "~S" form))
  (eval '(defbackend bk-br-p (:machine callfoo) (branches call ret)))
  (eval '(defbackend bk-br-c (:extends bk-br-p) (branches call)))
...
(fiveam:test stack-writers-are-derived-from-instruction-semantics
  ...
  (fiveam:is (equal '(("ADDS" "CALL-SPI") ("MOVSF") ("POPFP") ("POPR" "CALL-REG") ("PUSH" "CALL-IMM")
                      ("PUSHFP") ("PUSHV" "CALL-FP-IDX" "CALL-IMM" "CALL-REG" "CALL-SP-IDX") ("SUBS" "CALL-SPI"))
  ...
  (flet ((written (mnemonic)
    ...
    (fiveam:is (equal '(nil nil nil) (written "MOVV")))
...
(fiveam:test a-stack-writers-clause-adds-and-excepts
  ...
  (fiveam:is (equal '(("ADDS" "CALL-SPI") ("MOVV" "CALL-RI" "CALL-RR" "CALL-RS") ("POPR" "CALL-REG") ("SUBS" "CALL-SPI"))
...
(fiveam:test a-child-without-op-the-parent-dropped-still-rebuilds
  (eval '(defbackend bk-wo-parent (:machine callfoo)
          (operands (reg call-reg) (imm call-imm))
          (ops (:push (x) (pushv x)) (:pop (x) (popr x)))))
  (eval '(defbackend bk-wo-child (:extends bk-wo-parent) (without-ops pop)))
  (fiveam:is (typep (%backend-error-of '(defbackend bk-wo-direct (:extends bk-wo-parent) (without-ops nope)))
                    'backend-definition-error))
  (let ((warned nil))
    (handler-bind ((style-warning (lambda (c)
                                    (setf warned (search "bk-wo-child" (string-downcase (princ-to-string c))))
                                    (muffle-warning c))))
      (eval '(defbackend bk-wo-parent (:machine callfoo)
              (operands (reg call-reg) (imm call-imm))
              (ops (:push (x) (pushv x))))))
