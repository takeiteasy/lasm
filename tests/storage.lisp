...
(fiveam:test access-hook-sees-stack-accesses-by-slot
  (let* ((m (make-machine 'stack-test-machine))
    ...
    (stack-pop m 'ds)
    (fiveam:is (equal '((ds 0 :write 5) (ds :pointer :write 1)
                        (ds 1 :write 7) (ds :pointer :write 2)
                        (ds 0 :read 5)
                        (ds 1 :write 9)
                        (ds 1 :read 9) (ds :pointer :write 1))
                      (funcall drain)))))
...
(fiveam:test stack-push-and-pop-report-depth-to-access-hook
  (let ((m (make-machine 'test-machine))
        events)
    (setf (machine-access-hook m)
          (lambda (machine name index access value)
            (cl:push (list index access value (stack-depth machine name)) events)))
    (stack-push m 's 9)
    (stack-pop m 's)
