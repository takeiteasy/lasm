  ...
  (let ((descriptor (machine-descriptor machine)))
    (list :lasm-snapshot
          ...
          :interrupt-queue (mapcar (lambda (entry)
                                     (list* (and (first entry) (device-index (first entry)))
                                            (rest entry)))
    ...
    (let* ((saved (%snapshot-field snapshot :elements))
      ...
      (let ((count (length plan)))
        (dolist (entry (%snapshot-field snapshot :interrupt-queue))
          (unless (and (consp entry) (consp (cdr entry)) (consp (cddr entry)) (null (cdddr entry))
...
(defun %restore-snapshot (machine snapshot cells)
  ...
  (multiple-value-bind (values plan banks) (%validate-snapshot machine snapshot cells)
    ...
    (setf (machine-interrupt-queue machine)
          (mapcar (lambda (entry)
                    (list* (and (first entry) (aref (machine-devices machine) (first entry)))
                           (rest entry)))
                  (%snapshot-field snapshot :interrupt-queue))
          (machine-interrupt-active machine)
