...
;;;; DCPU-16's real layout, MSB-first: a 6-bit `av` field, a 5-bit `bv`
;;;; field, a 5-bit `opcode` field. `av`'s variant table is DCPU-16's real
;;;; one too -- -1..30 biased by 33 packs into 0x20..0x3f inline, anything
;;;; else escapes to its own following word (escape value 0x1f) -- see
;;;; examples/word.lisp for the mechanism itself on a byte-addressed
;;;; machine, and examples/wordaddr.lisp for word-addressed memory on its
;;;; own.
;;;;
...
;;;; that overload. DCPU16FOO's `av` field is a literal-or-escape only;
...
;;;;
;;;; #72: REG's bank cells carry DCPU-16's own register names (A B C X Y Z I
;;;; J) via :names, so source reads "addr a, b" rather than "addr 0, 1".
...
;;;; field AV is the signed offset from the address of the *next*
...
;;; DCPU-16's own A B C X Y Z I J via #72's :names), and one word-addressed
;;; (:cell-width 16) memory element sized to DCPU-16's own 16-bit address
;;; space.
...
;; SET dst, src -- dst is always a plain register index (field BV, full
;; 5-bit range, no variant needed). src packs a small literal inline into
;; field AV or escapes to its own word for a larger one.
...
;; ADD dst, src -- reg[dst] += src (a literal, same field-AV variant as SET).
...
;; variant (a small address packs inline, same as any other field-AV value;
...
;; before packing it into field AV. Unlike SET/ADD/STO's own field-AV table
...
;; the same field-AV escape mechanism serving a PC-relative use as it does
;; SET/ADD's literal one, just over its own signed range.
(defparameter *source*
  "set a, 5        ; reg[a] = 5, packs inline into AV
set b, 1000      ; reg[b] = 1000, does not fit -- AV escapes, value in its own word
addr a, b        ; reg[a] = reg[a] + reg[b] = 1005
sto result, a    ; RAM[result] = reg[a]
bra skip         ; PC-relative jump over the dead ADD below
add a, b         ; dead code -- skipped by BRA; reaching it would corrupt reg[a]
skip: hlt
...
(let ((assembly (assemble *source* :machine 'dcpu16foo)))
  ...
  (format t "  length:     ~D cells (1 each for SET A/ADDR/STO/BRA/ADD(dead)/HLT/RESULT, ~
2 for SET B's extra word)~%"
  ...
  (let ((m (make-machine 'dcpu16foo)))
    ...
    (multiple-value-bind (reason steps) (run m)
      ...
      (format t "  A = ~D (expected 1005 -- BRA must have skipped the dead ADD)~%"
              (regref m 'reg 0))
      (format t "  RAM[result] = ~D (expected 1005)~%"
              (mref m 'ram (gethash "result" (assembly-symbols assembly))))
      (assert (eq :trap reason))
      ;; SET A, SET B, ADDR, STO, BRA, HLT -- BRA's own jump skips the
