...
;;;;
;;;; A nested varying alternative with several varying ONE-OFs. NT-PAIR has
;;;; two, "lhs" and "rhs", each a bare address or "[base, offset]", and it sits
;;;; beside "#imm, value" in the outer NT-ANY.
;;;;
;;;; Options are trees, one subkey per varying ONE-OF:
;;;; (choice (nt-pair nt-idx nt-abs)) is an indexed lhs with a bare rhs.
;;;; FOR-CHOICE and CHOICE-CASE name each ONE-OF by its slot,
;;;; (for-choice (src nt-pair lhs nt-idx) ...) and
;;;; (choice-case (src nt-pair lhs) ...). Each extra operand lands at its own
;;;; ONE-OF's position, so `dst` below always binds the second address.
;;;;
;;;; Run with:  sbcl --script examples/nesttree.lisp

(load (merge-pathnames "boot.lisp" *load-pathname*))

(in-package #:lasm)

(defmachine nesttreefoo
...
(definstruction nesttreefoo add
  (modes nt-any)
  (encoding (opcode #x10)
            (operand src :width 1
  ...
  (semantics
    (choice-case src
      (nt-imm (set! a (+ src dst)))
      (nt-pair (choice-case (src nt-pair lhs)
                 (nt-abs (choice-case (src nt-pair rhs)
                           (nt-abs (set! a (+ src dst)))
                           (nt-idx (set! a (+ src dst roff)))))
                 (nt-idx (choice-case (src nt-pair rhs)
                           (nt-abs (set! a (+ src loff dst)))
                           (nt-idx (set! a (+ src loff dst roff))))))))))
...
(dolist (case '(("add 1, 3" #(#x10 #x00 #x01 #x03))
                ...
                ("add #1, 3" #(#x10 #x04 #x01 #x03))))
  (let ((cells (assembly-cells (assemble (first case) :machine 'nesttreefoo))))
...
(multiple-value-bind (descriptor values size choices)
    (decode-instruction-at (vector-cell-reader #(#x10 #x02 #x01 #x02 #x03)) 0 'nesttreefoo)
  (assert (string= "ADD" (instruction-descriptor-name descriptor)))
  (assert (equal '(src loff dst) (instruction-descriptor-operand-names descriptor)))
  (assert (equal '(1 2 3) values))
  (assert (= 5 size))
  (assert (every (lambda (choice) (equal '(nt-pair nt-idx nt-abs) choice)) choices)))

(format t "~%Running it -- extras and trailing operands keep their own bindings:~%")
(let ((m (make-machine 'nesttreefoo)))
  (dolist (case '(("add 1, 3" 4) ("add [1, 2], 3" 6) ("add 1, [3, 4]" 8)
                  ("add [1, 2], [3, 4]" 10) ("add #1, 3" 4)))
    (setf (sref m 'pc) 0)
    (load-program m (assemble (first case) :machine 'nesttreefoo))
    (step-machine m)
...
(dolist (text '("add 1, 3" "add [1, 2], 3" "add 1, [3, 4]" "add [1, 2], [3, 4]"))
  (let ((line (first (disassemble-assembly (assemble text :machine 'nesttreefoo)
                                           :machine 'nesttreefoo :labels nil :suffixes nil))))
